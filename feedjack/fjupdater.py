'''
-*- coding: utf-8 -*-
functions and classes for updating Feedjack (moved from bin/feedjack_update.py so we can import them properly)
...
def mtime(ttime):
    try:
        return datetime(*ttime[:6])
    except TypeError:
        # sometimes it seems the date isn't so parsed at all
        return datetime.strptime(ttime, '%a, %d %b %Y %H:%M:%S %Z')
