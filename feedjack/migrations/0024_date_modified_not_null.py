    ...
    def forwards(self, orm):
        ...
        while True:
            ...
            posts = list( orm['feedjack.Post'].objects\
                .order_by('id').filter(id__gt=id_pos, date_modified__isnull=True)\
                .select_for_update()[:bs] )
            if not posts: break
            for post in posts:
                # Not using "date_updated" here, as it might be mangled by e.g. other migrations
                post.date_modified = post.date_created
                post.save()
