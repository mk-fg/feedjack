...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option('-l', '--list', action='store_true', dest='list',
                    default=False, help="List available feeds"),
        make_option('-d', '--delta', dest='delta',
                    type="int", default=14,
                    help="The age in days to purge the entries (%default)"),
        make_option('--full', action='store_true', dest='full',
                    default=False, help="Purge feeds completely"),
    )
    usage = lambda foo, bar: ("%prog [feedname1] [feedname2] [options] "
    ...
    def handle(self, *feeds, **options):
        ...
        if (list):
            ...
            if feeds:
                ...
                if not_defined:
                    raise CommandError("Specified feeds not defined: " +
                                       ", ".join(not_defined))
            else:
